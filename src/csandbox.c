...
void __attribute__ ((constructor)) install_syscall_filter() {
  scmp_filter_ctx ctx = seccomp_init(SCMP_ACT_KILL);
  seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(read), 0); // FROM 0
  seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(write), 0);
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(open), -1);
  ...
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(getuid), -1); // FROM 102
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(getgid), -1);
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(geteuid), -1);
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(getegid), -1);
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(getppid), -1);
  seccomp_rule_add(ctx, SCMP_ACT_ERRNO, SCMP_SYS(getpgrp), -1); // TO 111
